  ...
  private initServer(): void {
    ...
    this.pixelAgentsServer
      ...
      .then((config) => {
        ...
        const stored = this.adapter.getSetting<boolean>(
          GLOBAL_KEY_HOOKS_ENABLED,
          providerSupportsHooks(this.provider),
        );
        const hooksEnabled = normalizeHooksEnabled(this.provider, stored);
        if (hooksEnabled !== stored) {
          this.adapter.setSetting(GLOBAL_KEY_HOOKS_ENABLED, hooksEnabled);
        }
        this.runtime.hooksEnabled.current = hooksEnabled;
        if (hooksEnabled) {
          void this.provider.installHooks(`http://127.0.0.1:${config.port}`, config.token);
          copyHookScript(this.context.extensionPath, this.provider);
        }
  ...
  resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      if (message.type === 'launchAgent') {
        ...
        const enabled = normalizeHooksEnabled(this.provider, message.enabled as boolean);
        ...
        const hooksEnabled = normalizeHooksEnabled(
          this.provider,
          this.adapter.getSetting<boolean>(GLOBAL_KEY_HOOKS_ENABLED, providerSupportsHooks(this.provider)),
        );
