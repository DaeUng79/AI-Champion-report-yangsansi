...
async function main(): Promise<void> {
  ...
  console.log(`[Pixel Agents] Active provider: ${provider.id}`);
  ...
  try {
    ...
    const onSetHooksEnabled = async (enabled: boolean): Promise<void> => {
      ...
      const normalized = normalizeHooksEnabled(provider, enabled);
      if (normalized) {
        await provider.installHooks(
          `http://127.0.0.1:${currentConfig.port}`,
          currentConfig.token,
        );
        copyHookScript(distRoot, provider);
    ...
    runtime.hooksEnabled.current = normalizeHooksEnabled(
      provider,
      adapter.getSetting('pixel-agents.hooksEnabled', providerSupportsHooks(provider)),
    );
    ...
    // Start scanning for external sessions from the active provider.
