...
export function handleClientMessage(
  ...
  const provider = runtime?.provider ?? getActiveProvider();
  ...
  switch (msg.type) {
    ...
    case 'setHooksEnabled': {
      const enabled = normalizeHooksEnabled(provider, msg.enabled as boolean);
...
function handleWebviewReady(send: WsSend, ctx: ClientMessageContext): void {
  ...
  const hooksEnabled = normalizeHooksEnabled(
    provider,
    adapter?.getSetting(KEY_HOOKS_ENABLED, providerSupportsHooks(provider)) ??
      providerSupportsHooks(provider),
  );
  send({
    type: 'settingsLoaded',
