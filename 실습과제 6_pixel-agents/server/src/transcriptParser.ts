...
export function processTranscriptLine(
  ...
  try {
    ...
    // Codex JSONL format (response_item/event_msg): detect tool start/end and task lifecycle.
    const payload =
    ...
    if (record.type === 'response_item' && payload) {
      ...
      if (itemType === 'function_call') {
        const toolId =
          typeof payload.call_id === 'string'
            ? payload.call_id
            : `codex-tool-${agentId}-${agent.linesProcessed}`;
      ...
      if (itemType === 'function_call_output') {
        const completedToolId = typeof payload.call_id === 'string' ? payload.call_id : '';
        if (completedToolId) {
          ...
          const useJsonlToolEvents = agent.hookDelivered && hasInlineTeammates(agentId, agents);
          if (!agent.hookDelivered || useJsonlToolEvents) {
    ...
    if (record.type === 'event_msg' && payload) {
      ...
      if (eventType === 'task_complete' && agent.activeToolIds.size === 0) {
